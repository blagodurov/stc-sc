...
# 
# Version 1.1
# Usage: python member-counting.py
# Instructions:
#     1. Specify your logins in global variables userFB, userLinkedIn,
#     userWeb (see below).
#     2. Launch the script. You'll be prompted for the passwords.
#     3. Wait until the script finishes. You will then find the log
...
#        exit(2)
    ...
    cur_url = 'http://www.facebook.com/groups/STC.Sustainable.Computing/members/'
    printToLog("fetching FB group members page: " + cur_url)
    res = browser.open(cur_url)
    parp = res.read()
    m = re.findall('www\.facebook\.com\/([a-zA-Z0-9\-\.?=]+)" data-hovercard', parp)
    res.close()
    # uniqueness:
    m = list(set(m))
    
    # adding names of the FB group members with inactive FB accounts to the contact list
    inactive_fb_accounts = re.findall('class="fsl fwb fcb"><span>([a-zA-Z0-9 ]+)<\/span>', parp)
    # uniqueness:
    ...
    """
    # mobile Facebook parsing (use if regular FB website does not work)
    base_page0 = "http://m.facebook.com/groups/260557033966377?view=members"
    base_page = 'http://m.facebook.com/browse/group/members/?id=260557033966377&start='
    m = []
    page = 0
    while True:
        if page == 0:
        ...
        res = browser.open(cur_url)
        parp = res.read()
        m = m + re.findall('"\/([a-zA-Z0-9\-\.]+)\?ref=pb', parp)
        m = m + re.findall('"\/([a-zA-Z0-9\-\.?=]+)&amp;ref=pb', parp)
        res.close()
        if re.search('See More', parp):
            if page == 0:
                page += 10
            else:
                page += 30
    ...
    for i in m:
        ...
        ms = re.search('<strong class="profileName">(.*?)<\/strong>', cont)
        if ms:
            full_name = ms.group(1)
        
        ms = re.search('mailto:(.*?)"', cont)
        if ms:
            email = ms.group(1).replace("%40", "@")

        ms = re.search('<span class="c mfss">(.*?)<\/span>', cont, re.S)
        if ms:
...
def mainLinkedIn():
    ...
    # as taken from the action attribute of the login form:
    res = browser.open('http://www.linkedin.com/uas/login-submit', data)
    rcode = res.code
    printToLog(rcode)
#    if not re.search('home\.php$', res.url):
#        print 'Login Failed'
#        exit(2)
    ...
    base_page = 'http://www.linkedin.com/groups?viewMembers=&gid=4092681'
    ...
    while True:
        ...
        res = browser.open(cur_url)
        parp = res.read()
        m = m + re.findall('"\/profile\/view\?id=([0-9]+)&', parp)
        res.close()
        #break
        if re.search('<strong>next', parp):
            page += 1
        else:
            break
    # uniqueness:
    ...
    for i in m:
        ...
        for j in range(1, 3):
            ...
            ms = re.search('<span class="given-name">(.*?)<\/span>', cont)
            if ms:
                first_name = ms.group(1)
            
            ms = re.search('<span class="family-name">(.*?)<\/span>', cont)
            if ms:
        ...
        ms = re.search('<p class="headline-title title" style="display:block">(.*?)<\/p>', cont, re.S)
        if ms:
            affiliation = ms.group(1).replace("\n", "").strip()
...
def mainWeb():
    ...
    spreadsheet_id = "0AjUwI2hDSR6_dC1KSXFqVldGM1NSWnFLYnppZmY0S0E" # (spreadsheet key here)
...
def appendToContact(timestamp, full_name, linkedin_name, email, why_join, affiliation, others, group):
    ...
    if linkedin_name == '':
        printToLog("Warning: parsing in appendToContact (linkedin_name is empty, full_name is " + full_name + ")!")
        print "Warning: parsing in appendToContact (linkedin_name is empty, full_name is " + full_name + ")!"
        return
